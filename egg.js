...
/**
 * The egg namespace contains everything needed to parse and evaluate egg
 * 
 * @namespace egg
 */
...
/**
 * 
 * @param {string} program is a string containing the egg program to be parsed
 * @returns {string} whatever parseApply returns
 * @throws {SyntaxError} when the expression/program to parse is not recognized as a string, number or word
 */
egg.parseExpression =function(program) {
  program = this.skipSpace(program);
  let match, expr;
  if (match = /^"([^"]*)"/.exec(program)) {  //string--starts with ", then 0 or more chars that are not ", then "
    expr = { type: "value", value: match[1] };
  } else if (match = /^\d+\b/.exec(program)) {  //number--starts with 1 or more digits then a word boundary (not digit or letter)
    expr = { type: "value", value: Number(match[0]) };
  } else if (match = /^[^\s(),#"]+/.exec(program)) {  //word--1 or more chars that are not whitespace, parens, comma, hash, quote
...
/**
 * 
 * @param {string} string can be any string
 * @returns {string} the input string with white space removed from front
 */
egg.skipSpace = function(string) {
  let first = string.search(/\S/); //finds first character that is not whitespace, or -1 if none
  if (first == -1) return "";
...
/**
 * 
 * @param {string} expr is a string containing an expression from the egg code
 * @param {string} program is a string containing the remaining unparsed portion of the program
...
/**
 * The code to create an array in Egg
 * 
 * @memberof egg.topScope
 * 
 * @param {...*} values var-arg values
...
/**
* The code for the Egg length function that returns the length of an array
* 
* @memberof egg.topScope
* 
* @param {Array} array the array which we want the length of
* @returns {number} the length of the array
*/
egg.topScope.length = function (array) {
  return array.length;
};
/**
* The code for the Egg element function that returns an element from the array
* 
* @memberof egg.topScope
* 
* @param {Array} array the array we want an element from
* @param {number} n the index of the element we want
* @returns {*} the element that was stored at the given index
*/
egg.topScope.element = function (array, n) {
  return array[n];
};
